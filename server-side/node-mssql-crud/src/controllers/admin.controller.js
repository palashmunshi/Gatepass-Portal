    ...
    try {
        ...
        var ws = wb.addWorksheet('student-tenure-wise');
            ...
            ws.cell(4,2)
                .string('Request No.');
            ws.cell(4,3)
                .string('Gatepass Requested'); //convert gate type no to gatepass name
            ws.cell(4,4)
                .string('Departure Date'); //convert time into readable format
            ws.cell(4,5)
                .string('Departure Time');
            ws.cell(4,6)
                .string('Actual Departure Date');
            ws.cell(4,7)
            ...
            for(var x in records){
                ...
                var from_date = String(temp.getDate()+'-'+temp.getMonth()+'-'+temp.getFullYear());
                var temp = new Date(records[x].actual_out_date);
                var actual_out_date = String(temp.getDate()+'-'+temp.getMonth()+'-'+temp.getFullYear());
                var temp = new Date(records[x].to_date);
                var to_date = String(temp.getDate()+'-'+temp.getMonth()+'-'+temp.getFullYear());
                var temp = new Date(records[x].actual_in_date);
                var actual_in_date = String(temp.getDate()+'-'+temp.getMonth()+'-'+temp.getFullYear());
                var temp = new Date(records[x].applied_date);
                var applied_date = String(temp.getDate()+'-'+temp.getMonth()+'-'+temp.getFullYear());
                ...
                .number(i-4);
                ws.cell(i,2)
                    .number(records[x].request_id);
                ws.cell(i,3)
                    .number(records[x].gatepass_type); //convert gate type no to gatepass name
                ws.cell(i,4)
                    .string(from_date); //convert time into readable format
                ws.cell(i,5)
                ...
                ws.cell(i,9)
                    ...
                    .style({numberFormat: 'hh:mm:ss'});
                ws.cell(i,10)
                    .string(actual_in_date);
                ws.cell(i,11)
                    .date(records[x].actual_in_time)
                    .style({numberFormat: 'hh:mm:ss'});
                ws.cell(i,12)
                    .string(records[x].status);
                ws.cell(i,13)
                    .string(records[x].send_approval_to);
                ws.cell(i,14)
                    .string(records[x].approved_or_rejected_by);
                ws.cell(i,15)
                    .string(records[x].destination);
                ws.cell(i,16)
                    .string(records[x].purpose);
                ws.cell(i,17)
                    .string(applied_date);
                ws.cell(i,18)
                    ...
                    .style({numberFormat: 'hh:mm:ss'});
                ws.cell(i,19)
                    .string(records[x].check_out_by);
                ws.cell(i,20)
                    .string(records[x].check_in_by);
                i=i+1;
...
export const createBlacklistedStudent = async (req, res) => {
    ...
    // validating
    if (user_id ==null || from_date==null || from_time==null || to_date==null || to_time==null || remark==null) {
        return res.status(400).json({ msg: "Bad Request. Please fill all fields" });
    };
    ...
    res.json({user_id,from_date,from_time,to_date,to_time,blacklisted_by,remark,visibility})
