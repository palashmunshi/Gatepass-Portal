...
export const queries = {
    ...
    updateDefaulterFlag:"UPDATE g[gps_db].[gps_db].[gps_gatepassmaster] SET defaulter_flag = CASE WHEN TRY_CAST(actual_in_date AS DATE) < TRY_CAST(to_date AS DATE) OR (TRY_CAST(actual_in_date AS DATE) = TRY_CAST(to_date AS DATE) AND TRY_CAST(actual_in_time AS TIME) <= TRY_CAST(to_time AS TIME)) THEN 0 ELSE 1 END where user_id=@user_id AND request_id=@request_id and status = 'checkedin'; ",
