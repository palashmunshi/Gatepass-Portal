...
const EmergencyGPform = (props) => {
  ...
  const checkEmergencyGP = async () => {
    ...
    if (res3 != 0) {
      alert(
        "Cannot Apply: You have already applied for a Emergency Gatepass"
      );
      return false;
    }
     else {
  ...
  const applyEmergencyGatepass = async () => {
    let data = { ...formInput };
    const id = data["user_id"];
    let fetchData = fetch(
      "",
  ...
  const handleClick = async (event) => {
    ...
    const check = await checkEmergencyGP();

    if (check == true) {
      await applyEmergencyGatepass();
  ...
  return (
    <div className="lfform">
      <form className="form">
        ...
        <div className="common">
          <label className="label">Visit to</label>
          <input
            type="text"
            name="visit_to"
            disabled={false}
...
export default EmergencyGPform;
